...
class NN:
    ...
    def _make_pipeline(self,n_comp,h,lr,reg):
        steps = [('scaler',StandardScaler()),('NN',MLPClassifier(hidden_layer_sizes=h,activation='logistic',solver='adam',learning_rate='adaptive',learning_rate_init=lr,alpha=reg,max_iter=500))] #('pca',PCA(n_components=n_comp))
        pipe = Pipeline(steps)
        return pipe
